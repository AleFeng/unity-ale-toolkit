...
| **UI** | Virtual-scroll lists (grid / sequential, pooled + render-only-visible), tab strips, filter bars, a tooltip base, a right-click context menu, a modal-popup base, an item instance pool, and other general widgets |
...
| **Effect system (`Ale.Effect`)** | A UE5-GAS-`GameplayEffect`-style effect system: `EffectDefinition` (duration / period / stacking / tags / modifiers / phase executions) + the runtime `EffectContainer`; since 1.10.0 it ships the effect library `EffectDatabase` shared by every upper system plus the Effect Editor — upper layers store only effect ids and implement their own auto-discovered `[EffectExecutor]` executors. Engine-agnostic Core |
...
| `Ale.Effect.Core` / `.Runtime` / `.Editor` | Effect system: engine-agnostic model · container · runner · registry · JSON / Unity bridge + shared effect library `EffectDatabase` + startup bootstrap / Effect Editor + catalog + reference-list drawer + welcome window | Core references `Ale.Condition.Core` + `Ale.GameplayTags.Core` + `Ale.Modifier.Core` + Newtonsoft; Runtime references `Ale.Toolkit.Runtime` (since 1.10.0) |
