...
- **Effect Editor**（`Tools > Ale Toolkit > Effect System > Effect Editor`、またはアセットの Inspector の「Effect Editor で編集」）：左列は効果テンプレート / ゲームプレイタグ / 列挙型、中列は効果リスト（テンプレートフィルター / 検索 / テンプレートから追加 / クイック追加）、右列は重複チェック付き ID + 名前 / 説明 / アイコン + カスタム属性 + インライン定義 + 検証サマリー。重複はエクスポートをブロック、JSON / バイナリでエクスポート。`EffectEditorWindow.Open(db, effectId)` で特定の効果に位置決め。
