...
{
    /// <summary>
    /// 单个条件参数值（<c>AttributeValue</c> 的极简化：仅标量 5 型 + 数组）。
    ...
    {
        /// <summary>参数标识（与判定器 <see cref="ConditionParamDef.id"/> 对应）。</summary>
        public string id;

        /// <summary>值类型。</summary>
        public ConditionParamType type;
        ...
        public ConditionParam(string id, ConditionParamType type, bool isArray = false, string enumTypeRef = null)
        {
            this.id          = id;
            this.type        = type;
        ...
        public int Count
        {
            get
            {
                switch (type)
                {
                    case ConditionParamType.String: return strings.Count;
                    case ConditionParamType.Float:  return floats.Count;
        ...
        public ConditionParam Clone()
        {
            return new ConditionParam(id, type, isArray, enumTypeRef)
            {
