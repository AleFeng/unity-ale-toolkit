...
{
    /// <summary>
    /// 编辑期判定器目录：用 <see cref="TypeCache"/> 发现所有带 <see cref="ConditionEvaluatorAttribute"/> 的判定器，
    /// 缓存 Key→实例，提供按 Category 分组的下拉菜单 + 参数区按 schema 同步。
    /// </summary>
    public static class ConditionEvaluatorCatalog
    {
        private static Dictionary<string, IConditionEvaluator> _byKey;
        private static List<IConditionEvaluator> _all;

        private static void EnsureBuilt()
        {
            if (_byKey != null) return;
            _byKey = new Dictionary<string, IConditionEvaluator>();
            _all   = new List<IConditionEvaluator>();

            foreach (var t in TypeCache.GetTypesWithAttribute<ConditionEvaluatorAttribute>())
            {
                if (t.IsAbstract || t.IsInterface) continue;
                if (!typeof(IConditionEvaluator).IsAssignableFrom(t)) continue;
                if (t.GetConstructor(Type.EmptyTypes) == null) continue;

                var inst = Activator.CreateInstance(t) as IConditionEvaluator;
        ...
        /// <summary>全部已发现判定器（按 Category、Key 排序）。</summary>
        public static IReadOnlyList<IConditionEvaluator> All { get { EnsureBuilt(); return _all; } }

        /// <summary>按键取判定器，未找到返回 null。</summary>
        public static IConditionEvaluator Get(string key)
        {
            EnsureBuilt();
        ...
        {
            if (string.IsNullOrEmpty(key)) return "(选择判定器)";
            var e = Get(key);
        ...
        public static GenericMenu BuildKeyMenu(string currentKey, Action<string> onSelect)
        {
            ...
            foreach (var ev in _all)
            {
                ...
                string label = $"{cat}/{ev.DisplayName}";
                string key   = ev.Key;
                menu.AddItem(new GUIContent(label), key == currentKey, () => onSelect?.Invoke(key));
            }
            if (_all.Count == 0)
                menu.AddDisabledItem(new GUIContent("（未发现任何判定器）"));
            return menu;
        }

        /// <summary>
        /// 让 <c>itemProp.parameters</c> 与所选判定器 schema 对齐：删除 schema 外的参数、补缺失参数、
        /// 类型 / 数组形态 / 枚举引用不符则重置。直接改 <see cref="SerializedProperty"/>，Undo 由 SerializedObject 自动处理。
        /// </summary>
        public static void SyncParameters(SerializedProperty itemProp, IReadOnlyList<ConditionParamDef> schema)
        {
            if (itemProp == null) return;
            var paramsProp = itemProp.FindPropertyRelative("parameters");
            if (paramsProp == null) return;
            schema = schema ?? Array.Empty<ConditionParamDef>();
        ...
        private static void ResetParam(SerializedProperty el, ConditionParamDef def)
        {
            ...
            if (!def.isArray) // 标量：初始化一个默认元素以便直接编辑
            {
                switch (def.type)
                {
                    case ConditionParamType.String: strings.arraySize = 1; break;
                    case ConditionParamType.Float:  floats.arraySize  = 1; break;
