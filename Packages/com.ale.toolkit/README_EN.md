...
- **Effect Editor** (`Tools > Ale Toolkit > Effect System > Effect Editor`, or "Edit in Effect Editor" on the asset's inspector): left column effect templates / gameplay tags / enum types, middle column the effect list (template filter / search / add from template / quick add), right column duplicate-checked ID + name / description / icon + custom attributes + inline definition + validation summary; duplicates block export; export as JSON / binary. `EffectEditorWindow.Open(db, effectId)` locates a specific effect.
