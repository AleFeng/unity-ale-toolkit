...
| **UI** | Virtual scrolling lists (grid / sequential, object pool + visible-region-only rendering; cell assign / recycle fade-in-out driven generically by `UiwListFadeCell` + the engine's default hooks), tab strips, filter bars, tooltip base classes, a right-click context menu `UiwContextMenu`, a modal-popup base `UiwModalPopupBase` (since 1.13.0), widget pools |
...
| **Condition System** | Data-driven two-level AND/OR conditions: declare a `ConditionExpression` field to configure it inline in the Inspector; upper layers implement `[ConditionEvaluator]` evaluators that are auto-discovered, and can reuse the ready-made comparison-operator kit and evaluation context. The engine-agnostic Core is server-side ready; **since 1.12.0** it also ships the condition library `ConditionDatabase` shared by every upper system (condition entries carry a display name / description / icon, template-driven custom attributes and the expression) plus the Condition Editor, so conditions can be referenced by id across systems |
...
| `Ale.Condition.Runtime` | Condition System · Unity bridge (`ConditionAsset` + auto-register on startup) plus the shared condition library `ConditionDatabase` / data manager / config serialization (since 1.12.0) | References `Ale.Toolkit.Runtime` since 1.12.0 |
| `Ale.Condition.Editor` | Condition System · inline drawer / catalog / Welcome window / parameter-candidate hook, plus the two-tab Condition Editor (since 1.12.0) | References `Ale.Toolkit.Runtime` + `Ale.Toolkit.Editor` since 1.12.0 |
...
**④ The condition library and the Condition Editor (since 1.12.0)**

Besides living inline in a host field, a condition can be configured as a **named entry with an id** and shared by several systems — "strength ≥ 10 and has the Brave trait" no longer has to be re-authored in traits, professions, titles and skill trees separately. The carrier is `ConditionDatabase` (`Create > Ale > Condition > Condition Database`) with three top-level lists: **condition entries** `ConditionEntry` (`id` + display name / description / icon as three `AttributeValue`s + template-schema custom attributes `values` + the embedded `expression`; the display fields feed the "requirement not met" UI directly), **condition templates** `ConditionTemplate` (`name` / `color` / custom-attribute schema + `defaultExpression`, deep-copied as a preset on create) and **enum types**. The library holds no gameplay tags — those are declared by the effect library.

```csharp
// Runtime: auto-registered from Resources on startup (ConditionRuntime.AutoLoadFromResources, default true), or register explicitly
ConditionDataManager.Instance.Register(conditionDatabase);   // idempotent; NormalizeAll + joins the global named-condition registry
bool ok = ConditionResolver.IsSatisfied("knight_ready", ctx); // resolution: context source → fallback source → ConditionDefinitionRegistry.Default
ConditionEntry e = ConditionDataManager.Instance.GetEntry("knight_ready");
string why = e.descriptionText.ResolveText();                 // the line to show the player when it is not met
```

- ⚠️ **Unresolved means "fails" (fail-closed)**: when `ConditionResolver.Evaluate` cannot find the id it returns not-passed, puts the id into `FailedKeys` and warns. For gating, a typo should lock content rather than unlock it — consistent with how `ConditionEngine` treats unregistered evaluator keys.
- Bootstrap: `[SubsystemRegistration]` clears the registries → `[BeforeSceneLoad]` only adds (evaluators + `Resources`), so a database a host registers in Awake survives.
- **Condition Editor** (`Tools > Ale Toolkit > Condition System > Condition Editor`, or "Edit in Condition Editor" on the asset's inspector) has two tabs, structurally symmetric to the Effect Editor:
  - **Condition Evaluators** (first tab; its content comes from **code**, so it works without any condition library): a catalog of every `IConditionEvaluator` implementation in the project, grouped by category, searchable by key / display name / full type name / assembly, with an "issues only" toggle. The right column shows the implementing type, assembly and source path plus "Open Script" (jumps to the class declaration line), "Show in Project" and "Copy Key" (double-clicking a row opens the script), the param schema, which configurations reference it ("Go" locates it), and **silent-failure diagnostics** — a missing `[ConditionEvaluator]`, an attribute key that disagrees with the `Key` property, a duplicate key, an empty key, a missing public parameterless constructor or a failed instantiation (an abstract base class is only noted, never flagged as a problem). A banner lists dangling keys that configurations reference but nothing implements. Usage sources cover the condition library, `ConditionAsset`, and **every effect's application condition and execution-item gates in the effect library** (contributed by the effect side through `ConditionEvaluatorIndex.RegisterUsageProvider`, so the dependency direction never reverses).
  - **Condition Database** (second tab): left column condition templates / enum types, middle column the condition list (template filter / search / add from template / quick add), right column duplicate-checked ID + name / description / icon + custom attributes + inline expression + validation summary (which reports "no evaluator selected" and "evaluator X has no implementation" on the spot); duplicates block export; export as JSON / binary. `ConditionEditorWindow.Open(db, conditionId)` locates a specific condition, `OpenEvaluators()` switches to the first tab.
- **Host integration** (one line in an inspector): `EditorConditionRefListDrawer.Draw(ctx, skill.unlockConditionRefs, drag, "Unlock conditions", "condition")` — catalog menu (grouped by database), drag-reorder, "Open" jump, "(not found)" marking (non-blocking), free-form input.
- **Wiring a host into the cross-reference**: conditions may stay inline in host fields — write a usage provider and they still show up in the Condition Evaluators tab's references / dangling keys / diagnostics: call `ConditionEvaluatorIndex.RegisterUsageProvider(Provide)` from an `[InitializeOnLoad]` static constructor, and let `Provide` walk your own assets with `ConditionUsageCollector.ForEachAsset<T>` and collect each site with `ConditionUsageCollector.Collect(into, expr, asset, ownerId, "Unlock condition", jump)` (the helper formats the location uniformly). The host supplies `jump` (open its own editor window and locate the entity), so the Condition System never has to know about the host. toolkit's own `EffectConditionUsageProvider` is written exactly this way.
- **Parameter candidates**: an evaluator declares in its schema what a string parameter holds via `catalogRef` (e.g. `new ConditionParamDef("traitId", ConditionParamType.String, false, "Trait ID", null, null, "Chronicle.Trait")`); the host supplies candidates for that catalog through `ConditionDrawerHooks.RegisterProvider(IConditionParamCatalogProvider)`, and the drawer turns the bare text field into a dropdown grouped by system name (a dangling value is kept and marked "(unknown)"). With no `catalogRef` or no candidates the behaviour is exactly as in 1.11.0.
